...
namespace Application.UseCases.Auctions.Handlers
{
    public class CreateAuctionHandler
    {
        ...
        public async Task<int> HandleAsync(CreateAuctionCommand command, CancellationToken cancellationToken = default)
        {
            ...
            var utcStartDate = dto.StartDate.Kind == DateTimeKind.Unspecified
                ? DateTime.SpecifyKind(dto.StartDate, DateTimeKind.Utc)
                : dto.StartDate.ToUniversalTime();

            var utcEndDate = dto.EndDate.Kind == DateTimeKind.Unspecified
                ? DateTime.SpecifyKind(dto.EndDate, DateTimeKind.Utc)
                : dto.EndDate.ToUniversalTime();

            var nowUtc = DateTime.UtcNow;

            if (utcStartDate < nowUtc.AddMinutes(-1))
            {
                throw new InvalidAuctionDateException("La fecha de inicio de la subasta no puede ser anterior a la fecha y hora actual.");
            }
            ...
            var auction = new Auction
            {
                ...
                EndDate = utcEndDate,
                Status = utcStartDate <= nowUtc ? "ACTIVA" : "PROGRAMADA",
